...
SELECT name, population 
   FROM world
WHERE name IN ('France','Germany','Italy')    /* IN() is used to find  matching in the list */
...
SELECT name, round((population/1000000),2), round((gdp/1000000000),2)
   FROM world
WHERE continent = 'South America'       /* using ROUND(number, decimal places) */                                
...
SELECT name, round((gdp/population), -3)    /* round(num, -3) for round of to 1000 */
   FROM world
...
SELECT name, capital
   FROM world
WHERE LEFT(capital, 1)=LEFT(name, 1) AND capital<>name   /* LEFT("ADITYA", 3) ==> "ADI" */
